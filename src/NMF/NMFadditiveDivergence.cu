...
#include "NMFadditiveDivergence.h"
...
void NMF_AdditiveDivergence::DoIteration(bool updateW) {
	int n = V.Rows();
	int m = V.Columns();
	int r = W.Columns();
	...
	UpdateH_AD<<<gh, bh>>>(H.Pointer(), W.Pointer(), V.Pointer(), WH.Pointer(),sum.Pointer(), n, m, r);
	...
	UpdateW_AD<<<gw, bw>>>(W.Pointer(), H.Pointer(), V.Pointer(), WH.Pointer(), sum.Pointer(), n, m, r);
}
