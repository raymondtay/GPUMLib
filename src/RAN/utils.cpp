...
namespace UTILS{
	...
	DeviceMatrix<float> pseudoinverse(DeviceMatrix<float> &Input){
		...
		DeviceMatrix<float> d_Sigma(N,M,ColumnMajor);
		...
		DeviceMatrix<float> sigmainverse(N,M,ColumnMajor);
		DeviceMatrix<float> aux2(N,M,ColumnMajor);
	...
	void pseudoinverse2(DeviceMatrix<float> &Input){
		...
		cublasSgemm('t', 'n', N, M, N, 1,     VT.Pointer(), LDVT, d_Sigma.Pointer(), N, 0, aux2.Pointer(), N);
		cublasSgemm('n', 't', N, M, M, 1, aux2.Pointer(), N,    U.Pointer(), M, 0, Input.Pointer(), N);

	}


	void pseudoinverse(DeviceMatrix<float> &Input,cudafloat* S, cudafloat* U, cudafloat* VT, cudafloat* d_Sigma,cudafloat* aux, cudafloat* output){
		...
		cudaMemset(S,0,sizeof(float)*imin(M,N));
		cudaMemset(U,0,sizeof(float)*LDU*M);
		cudaMemset(VT,0,sizeof(float)*LDVT*N);

		checkStatus(culaDeviceSgesvd(jobu, jobvt, M, N, Input.Pointer(), LDA, S, U, LDU, VT, LDVT));

		//checkStatus(status);

		cudaMemset(output,0,sizeof(float)*N*M);
		cudaMemset(aux,0,sizeof(float)*N*M);
		cudaMemset(d_Sigma,0,sizeof(float)*N*M);

		KernelSigmaInverse(d_Sigma,N,M,S);

		cublasSgemm('t', 'n', N, M, N, 1,     VT, LDVT, d_Sigma, N, 0, aux, N);
		cublasSgemm('n', 't', N, M, M, 1, aux, N,    U, M, 0, output, N);
